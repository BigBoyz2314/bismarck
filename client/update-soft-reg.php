<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if ($row1 == 0) {
            $stmt2 = "INSERT INTO `offices` (`user_id`,  `company_contact_name`, `company_name`, `company_address`, `company_city`, `company_state`, 
            ...
            `os_version`,`printer_ip_address`,`ip_address`
            )
			VALUES ('$id', '$company_contact_name', '$company_name', '$company_address', '$company_city', '$company_state',
			'$company_zip', '$company_contact_phone','$company_contact_email', '$company_fax', '$company_tax_id', '$preparer_id', '$preparer_name', 
			'$preparer_company_name', '$preparer_address', '$preparer_city', '$preparer_state', '$preparer_zip', '$preparer_phone',
			'$preparer_efin', '$preparer_ptin', '$preparer_nytprin', '$preparer_ein', '$preparer_ero_pin', '$preparer_pin',
            '$os_version','$printer_ip_address','$ip_address'
            ...
            if ($result2) {
                ...
                header("Location:software-reg.php?office=added");
                // echo $result2;
            }
            else {
            ...
            $stmt3 = "UPDATE `offices` SET `company_contact_name` = '$company_contact_name', `company_name` = '$company_name', `company_address` = '$company_address',
			`company_city` = '$company_city', `company_state` = '$company_state', `company_zip` = '$company_zip', 
			`company_contact_phone` = '$company_contact_phone',`company_contact_email` = '$company_contact_email',
			...
			`preparer_pin` = '$preparer_pin',  `os_version` = '$os_version' ,
             `printer_ip_address`='$printer_ip_address' , `ip_address`='$ip_address' WHERE `user_id` = '$id'";

            $result3 = mysqli_query($conn, $stmt3);
            if ($result3) {
                echo "Office information updated successfully";
                header("Location:software-reg.php?office=updated");

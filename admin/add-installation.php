            ...
            if(!empty($_FILES["file-es"]["name"])) {
                ...
                if (in_array($fileType1, $allowTypes) && in_array($fileType2, $allowTypes)) {
                    ...
                    if (move_uploaded_file($_FILES["file-en"]["tmp_name"], $targetFilePath1) &&
                        ...
                        $year = $_POST['year']; // Fetch text input value
                        $query = "INSERT INTO installation (data, year, file_en, file_es, created_at, updated_at) VALUES ('$textInput', '$year', '$file1Name', '$file2Name', NOW(), NOW())";
